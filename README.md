...
A headless DatePicker built for the GilaSoftware technical challenge.
...
| ---------- | ---------------------------------- | ----------------------- |
| **Engine** | `src/engine/DatePickerEngine.ts`   | Pure TS                 |
...
The engine uses `Temporal.PlainDate` and `Temporal.PlainYearMonth` exclusively.
...
**Browser support:** Chrome 121+, Firefox 139+, Safari 17.4+. The test environment (jsdom) lacks Temporal, so `src/__tests__/setup.ts` provides a minimal hand-written shim covering only the ~10 methods the engine actually calls.
...
src/
  engine/
    ...
    DatePickerEngine.ts      # Pure TS calendar logic.
  composables/
  ...
  __tests__/
    ...
    DatePickerEngine.spec.ts # Engine unit tests (pure TS)
